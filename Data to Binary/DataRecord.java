/**
 * @description This is the supporting class for the Prog1A and Prog1B. 
...
public class DataRecord {
	...
	public void dumpObject(RandomAccessFile stream) {
		// paddable strings
		StringBuffer date = new StringBuffer(this.dateTimeOfCall);
		StringBuffer group = new StringBuffer(this.animalGroupParent);
		StringBuffer padWard = new StringBuffer(this.ward);
		try {
			date.setLength(dateLength); // pad in spaces
			stream.writeBytes(date.toString());
			stream.writeInt(hourlyNotionalCost);
			stream.writeInt(incidentNotionalCost);
			group.setLength(groupLength); // pad in spaces
			stream.writeBytes(group.toString());
			stream.writeInt(wardCode);
			padWard.setLength(wardLength); // pad in spaces
			stream.writeBytes(padWard.toString());
